...
public class HistoryService {
    ...
    public HistoryService(HistoryRepository historyRepository, MemberRepository memberRepository, LicenseRepository licenseRepository) {
...
//    dodawanie ręczne rekordu do historii
    ...
    void addCompetitionRecord(UUID memberUUID, CompetitionMembersListEntity list) {
        ...
        if (historyEntity.getCompetitionHistory() != null) {
            ...
            String[] newState = new String[historyEntity.getCompetitionHistory().length + 1];
            for (int i = 0; i <= historyEntity.getCompetitionHistory().length - 1; i++) {
                newState[i] = historyEntity.getCompetitionHistory()[i];
                newState[i + 1] = list.getAttachedTo().concat(" " + list.getName().toUpperCase().substring(0, 1));
            }
